...
describe('Exec should be an async operation', () => {
  test('getting as input a file command and an optional args argument', async () => {
    expect.assertions(1);

    execFile.mockImplementation((file, args, cb) => cb(null, 'v14.6.1', null));

    await expect(exec('node', ['--version'])).resolves.toBeDefined();
  });

  test('where the file command arg is mandatory', async () => {
    expect.assertions(2);
    ...
    expect(execFile).not.toBeCalled();
  });

  test('where the optional args argument should be an array of strings', async () => {
    expect.assertions(2);
