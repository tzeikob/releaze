...
async function tag (version, message) {
  ...
  if (global.verbose) {
    logger.info('File package.json has been staged', 1);
  }
  ...
  try {
    ...
    if (global.verbose) {
      logger.info('File CHANGELOG.md has been staged', 1);
    }
  } catch (error) {}
  ...
  try {
    ...
    if (global.verbose) {
      logger.info('File package-lock.json has been staged', 1);
    }
  } catch (error) {}
  ...
  try {
    ...
    if (global.verbose) {
      logger.info('File npm-shrinkwrap.json has been staged', 1);
    }
  } catch (error) {}
  ...
  if (global.verbose) {
    logger.info('Staged files have been committed:', 1);

    const branchWithCommitHash = stdout.split('\n')[0];
    logger.info(branchWithCommitHash, 2);
  }
