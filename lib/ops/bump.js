...
async function bump (type, preid) {
  ...
  if (global.verbose) {
    logger.info('File package.json has been updated to new version:', 2);
    logger.info(`{ "version": "${current}" } \u2192 { "version": "${next}" }`, 4);
  }
  ...
  try {
    ...
    if (global.verbose) {
      logger.info('File package-lock.json has been updated to new version:', 2);
      logger.info(`{ "version": "${current}" } \u2192 { "version": "${next}" }`, 4);
    }
  } catch (error) {
  ...
  try {
    ...
    if (global.verbose) {
      logger.info('File npm-shrinkwrap.json has been updated to new version:', 2);
      logger.info(`{ "version": "${current}" } \u2192 { "version": "${next}" }`, 4);
    }
  } catch(error) {
