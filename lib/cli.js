...
const { parse, generateHelp } = optionator({
  ...
  options: [{
    ...
    heading: 'Run testing via npm test to prevent a release having failed tests'
  }, {
    option: 'test',
    type: 'Boolean',
    dependsOn: 'bump',
    description: 'Use this option to run tess via npm test before the release starts'
  }, {
...
async function run (args) {
  ...
  logger.success('All npm and git pre-conditions are met.', 2);

  if (opts.test) {
    logger.info('Running all unit and integration tests...');

    await exec('npm', ['test']);

    logger.success('All tests have been passed.', 2);
  }
  ...
  if (preid) {
    logger.info(`Bumping to next ${type} version with ${preid} preid...`);

    release = await bump(type, preid);
  } else {
    logger.info(`Bumping to next ${type} version...`);
  ...
  logger.success(`Version bumped from ${current} \u2933 ${type} ${next}.`, 2);
  ...
  if (opts.changelog) {
    ...
    logger.success('The CHANGELOG.md file has been updated.', 2);
  }

  if (opts.git) {
    logger.info('Creating a new bump release tag...');

    const { name, hash } = await tag(next, opts.message);

    logger.success(`Tag ${name} \u1F812 ${hash} has been created.`, 2);
  }

  logger.info('Release has been completed successfully.');
}
