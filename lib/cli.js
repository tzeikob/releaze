...
async function run (args) {
  ...
  if (opts.changelog) {
    ...
    const { filename, append } = await changelog(next, logs);

    if (append) {
      logger.success(`The ${yellow(filename)} file has been updated`, 1);
    } else {
      logger.success(`A new ${yellow(filename)} file has been created`, 1);
    }
  }
